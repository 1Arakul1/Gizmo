...
class Order(models.Model):
    ...
    return_request = models.OneToOneField('ReturnRequest', on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Запрос на возврат", related_name='order')
...
class ReturnRequest(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="Пользователь")
    order_item = models.ForeignKey('OrderItem', on_delete=models.CASCADE, verbose_name="Позиция заказа") # Связываем с конкретной позицией заказа
    ...
    comment = models.TextField(blank=True, null=True, verbose_name="Комментарий сотрудника") # Комментарий сотрудника при рассмотрении

    def __str__(self):
        return f"Запрос на возврат #{self.pk} от {self.user.username} - {self.order_item.item}"
