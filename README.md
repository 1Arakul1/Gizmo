...
*   **fps_data:** Отвечает за хранение и отображение данных FPS (кадров в секунду) для различных конфигураций компьютеров в играх. Включает модель `FPSData`, связывающую игры, процессоры, видеокарты и настройки графики с соответствующими значениями FPS.
*   **games:** Управляет информацией об играх, включая их названия, жанры и минимальные/рекомендуемые системные требования. Включает модель `Game` и представления для отображения списка игр и детальной информации о каждой игре.
...
**Шаблоны приложения `fps_data`:**

*   `fps_data_list.html`:  Список данных FPS (может использоваться для отладки).
*   `fps_data_detail.html`:  Детальная информация о данных FPS для конкретной игры и компонентов.

**Шаблоны приложения `games`:**

*   `game_list.html`: Список доступных игр.
*   `game_detail.html`: Детальная информация об игре.
*   `fps_form.html`: Форма для выбора компонентов и настроек для предсказания FPS.
*   `fps_result.html`: Результат предсказания FPS для выбранной конфигурации.
